...
class PresetTests(unittest.TestCase):
    ...
    def test_stub_edge_and_two_by_three_coupons(self):
        for edge, kind, expected in (("right", "rotary_perf", 150000-50800), ("bottom", "cross_perf", 120000-50800)):
        ...
        self.assertEqual([(o.kind, o.position_um) for o in ops],
                         [("rotary_perf", 30000), ("cross_perf", 40000), ("cross_perf", 80000)])
        with self.assertRaises(ValueError):
            preset_operations("coupons", 60000, 120000, columns=1, rows=1)

    def test_tent_two_halves_count_as_one_tool_with_combined_warning(self):
        ops = preset_operations("tent", 60000, 120000)
        self.assertEqual([(o.kind, o.position_um, o.start_um, o.end_um) for o in ops],
                         [("crease", 60000, 0, 0), ("strike_perf", 30000, 0, 60000),
                          ("strike_perf", 30000, 60000, 120000)])
